function [active_flow_ap2, index_flows, congested] = available_bitrate(BW, AP_SINR, selected_Rb_array, index_AP_array, ap2, bitrate, bitrate_AP)
...
if isempty(active_flow_ap2)
    ...
    active_flow_ap2(size+1) = Rb_total;
...
for aa=1:length(active_flow_ap2)
    if active_flow_ap2(aa) > max_dr_perflow && active_flow_ap2(kk) ~= -1
%         if sharing == 0
%            sharing = 1;
%         end
        active_flow_ap2(aa) = AP_capacity_to_be_shared/sharing;
    end
end
